...
class CERT(object):
    ...
    def parse(cls,buffer,length):
        ...
        try:
            start = buffer.offset
            (cert_type,labels,algorithm) = buffer.unpack("!HHB")
            cert = buffer.get(length-(buffer.offset-start))
            return cls(cert_type,labels,algorithm,cert)
        except (BufferError,BimapError) as e:
            raise DNSError("Error unpacking RD [offset=%d]: %s" %
                                    (buffer.offset,e))
    ...
    def fromZone(cls,rd,origin=None):
        ...
        # Unknown rata - assume hexdump in zone format
        # (DiG prepends "\\# <len>" to the hexdump so get last item)
        return(int(rd[0]),int(rd[1]),int(rd[2]),base64.b64decode(("".join(rd[3:])).encode('utf-8')))
        # return cls(binascii.unhexlify(rd[-1].encode('ascii')))

    def __init__(self,cert_type,labels,algorithm,cert):
        # Assume raw bytes
        # check_bytes('data',data)
    ...
    def pack(self,buffer):
        """
            Pack record into buffer
        """
        buffer.pack("!HHB",self.cert_type,self.labels,self.algorithm)
        buffer.append(self.cert)
    ...
    def __repr__(self):
        ...
        """
        # For unknown rdata just default to hex
        return "%d %d %d %s" % (self.cert_type, self.labels,self.algorithm,base64.b64encode(self.cert).decode())
    ...
    # Attributes for comparison
    attrs = ('cert_type','labels','algorithm','cert')
